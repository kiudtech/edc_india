...
export default function FellowshipApplicationPage() {
  ...
  const requestedPlanSlug = String(selectedPlan.slug || params.get('planSlug') || 'fellowship-program').trim().toLowerCase();
      ...
      for (const base of baseCandidates) {
        try {
          ...
          const matchedPlan = data.find((plan) => String(plan.slug || '').trim().toLowerCase() === targetSlug)
            || data.find((plan) => String(plan.ctaRoute || '').trim() === '/fellowship-application');
  ...
  const handleProceedToPayment = async (e) => {
    ...
    navigate('/payment', {
      state: { userId: authUserId, founderId: user.founderId, type: 'fellowship', planSlug, planName, successSubtitle: 'Your fellowship payment has been received successfully.' },
    });
  };
    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="relative flex flex-col justify-between overflow-hidden bg-gradient-to-br from-[#0f172a] via-[#1e3a5f] to-[#0f2d6b] px-8 py-12 text-white lg:w-[45%] lg:min-h-screen lg:px-12 lg:py-16">
        ...
        <div className="relative z-10 mt-12 rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
          ...
          <a href="/fellowship" className="mt-3 inline-flex items-center gap-1 text-xs text-white/50 hover:text-white transition">
            View more details →
          </a>
        </div>
      </div>
